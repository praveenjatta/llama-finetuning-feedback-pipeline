...
```bash
# all scripts live in the code/ folder — run everything from there
cd code
...
```bash
# 1. Clone the repo and move into the code/ folder
#    (all Python scripts and outputs.properties live in code/)
git clone https://github.com/praveenjatta/llama-finetuning-feedback-pipeline.git
cd llama-finetuning-feedback-pipeline/code
...
# 3. Set your Hugging Face token in the scripts (HF_TOKEN)

# 4. Download the base model (~16GB, 10 min)
python3.11 download_hf_model.py

# 5. Fine-tune both agents with LoRA (~2.5 hours on Apple Silicon)
python3.11 fine_tune_training.py

# 6. Merge adapters into base models
python3.11 merge_to_base_model.py

# 7. Start the fine-tuned model server
python3.11 load_run_local_finetune_model.py

# 8. In a new terminal, expose it via ngrok
ngrok http 8006

# 9. Update n8n LLM nodes with the ngrok URL
#    Classifier  -> {ngrok-url}/v2
#    InsightWriter -> {ngrok-url}/v3

# 10. Run evaluation (both phases)
export OPENAI_API_KEY="sk-proj-..."
