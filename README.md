...
| `test_feedback_engineering.py` | DeepEval evaluation suite with 8 test cases |
...
# 9. Run evaluation
python3.11 test_feedback_engineering.py
```
...
| Model | Vague Feedback Handling |
|-------|------------------------|
| Llama 3.1 base (before fine-tuning) | 75% |
| Llama 3.1 fine-tuned | **100%** |

Fine-tuning improved vague feedback handling by **25 percentage points** — same model, same architecture, same prompts. The only variable changed was the fine-tuning.

**Before fine-tuning**, "This app is garbage" produced invented bug details and fake reproduction steps.

**After fine-tuning**, the same input produces:
```
...
Groundedness on detailed feedback dropped slightly (to ~75%) because the fine-tuned model became more cautious — it occasionally adds "INSUFFICIENT DATA" disclaimers even on well-detailed tickets. This is a known consequence of training heavily on vague-refusal examples, and a candidate for the next fine-tuning iteration (balancing the training data).
